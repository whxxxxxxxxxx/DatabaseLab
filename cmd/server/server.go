...
func setUp() {
	...
	engine.GIN.Use(cors.New(cors.Config{
		...
		AllowMethods:     []string{"POST", "GET", "OPTIONS", "PUT", "DELETE"},                                                                // 允许使用的HTTP方法
		AllowHeaders:     []string{"Origin", "X-Requested-With", "Content-Type", "Accept", "Authorization", "refresh_token", "access_token"}, // 在这里添加 Authorization
